...
export const TERMO_RESPONSABILIDADE_VERSAO = "2026-07-03";
...
export class FuncionarioTermoService {
  gerar(input: { nome: string; cpf: string; aceitoEm: Date; foto: Buffer; assinatura: Buffer }) {
    ...
    this.text(page, `Versao: ${TERMO_RESPONSABILIDADE_VERSAO}`, 36, 765, 9);
    this.text(page, `Funcionario: ${input.nome}`, 36, 735, 11, true);
    this.text(page, `CPF: ${this.formatarCpf(input.cpf)}`, 36, 715, 10);
    this.text(page, `Aceite: ${this.formatarData(input.aceitoEm)}`, 36, 695, 10);
    this.text(page, "DECLARACAO", 36, 655, 11, true);
    const paragrafos = [
      ...
      "Comprometo-me a utilizar os equipamentos de protecao individual (EPIs) descritos no sistema e exigidos para cada atividade.",
      "Reconheco que minha identificacao, foto e assinatura poderao constar nos relatorios dos servicos executados por mim.",
      "Comprometo-me a nao compartilhar senha, acesso ou dados de clientes e a comunicar imediatamente qualquer incidente."
    ];
    let y = 625;
    ...
    this.text(page, "FOTO DO FUNCIONARIO", 55, 275, 9, true);
    this.text(page, "ASSINATURA DO FUNCIONARIO", 310, 275, 9, true);
    page.imagens = [
