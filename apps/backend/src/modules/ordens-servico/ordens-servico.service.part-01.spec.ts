...
test("finalizarOs agenda relatorio tecnico por email para cliente sem PMOC", async () => {
  ...
  assert.match(pdfText, /DADOS DA EMPRESA/);
  assert.match(pdfText, /DADOS DO CLIENTE/);
  assert.match(pdfText, /SERVICO EXECUTADO/);
  assert.match(pdfText, /Natureza do atendimento\s+Corretiva/);
  assert.match(pdfText, /Assinatura do Cliente\s+Maria Souza/);
  assert.match(pdfText, /Problema encontrado\s+Motor travado/);
  assert.match(pdfText, /Acao realizada\s+Motor destravado e testado/);
  assert.match(pdfText, /Pecas utilizadas\s+Produtos de limpeza/);
  assert.match(pdfText, /Observacao final\s+Funcionando/);
  assert.match(pdfText, /Fotos\s+Foto inicial: Foto do atendimento/);
