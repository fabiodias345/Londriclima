...
test("gerarPdfPmocCliente retorna PDF com nome de arquivo e conteudo oficial", async () => {
  ...
  assert.match(pdf, /RELAT.RIO T.CNICO PMOC/);
  assert.match(pdf, /Documento PMOC/);
  assert.match(pdf, /PLANO DE MANUTEN..O, OPERA..O E CONTROLE - PMOC/);
  assert.match(pdf, /Cliente\s+Maria Souza/);
  assert.match(pdf, /EMPRESA CONTRATADA/);
  assert.match(pdf, /M\. Lima Manuten..es Prediais e Industriais LTDA/);
  assert.match(pdf, /RESPONS.VEL T.CNICO/);
  ...
  assert.match(pdf, /RESUMO DAS M.QUINAS DO CLIENTE/);
  ...
  assert.match(pdf, /Manuten..o executada\s+Anual/);
  assert.doesNotMatch(pdf, /"Mensal", "Trimestral", "Semestral"/);
  assert.match(pdf, /M.QUINA N:001 - P.GINA EXCLUSIVA/);
  assert.match(pdf, /M.QUINA N:002 - P.GINA EXCLUSIVA/);
  const primeiraMaquinaInicio = pdf.search(/\(M.QUINA N:001/);
  const segundaMaquinaInicio = pdf.search(/\(M.QUINA N:002/);
  const paginaMaquina = pdf.slice(primeiraMaquinaInicio, segundaMaquinaInicio);
  assert.match(paginaMaquina, /Executado neste relat.rio/);
  assert.match(paginaMaquina, /Higieniza..o completa da condensadora/);
  assert.doesNotMatch(paginaMaquina, /\(Mensal\)|\(Trimestral\)|\(Semestral\)/);
  assert.doesNotMatch(paginaMaquina, /Periodicidade prevista/);
  assert.doesNotMatch(paginaMaquina, /PLANO DE MANUTEN..O DA M.QUINA|.LTIMA EXECU..O \/ OCORR.NCIAS/);
  assert.match(pdf, /.rea climatizada m2\s+N.o informado/);
  assert.match(pdf, /Ocupantes fixos\s+N.o informado/);
  assert.match(pdf, /Ocupantes vari.veis\s+N.o informado/);
  assert.match(pdf, /DECLARA..O T.CNICA E ASSINATURAS/);
  assert.match(pdf, /13\.589\/2018/);
  assert.match(pdf, /3\.523\/1998/);
  ...
  assert.ok(segundaMaquinaInicio < pdf.indexOf("(DECLARA"));
  const declaracao = pdf.slice(pdf.indexOf("(DECLARA"));
...
test("gerarPdfRelatorioAvulsoCliente mostra assinatura do cliente com legenda", async () => {
  const equipamento = criarEquipamentoPmocTeste("equipamento-1", "Sala", "AV-001", "SN-AV-1", "2026-06-11T12:00:00.000Z");
  const assinaturaPath = resolve(process.cwd(), "..", "..", "storage", "os", "os-assinatura-cliente", "assinatura-cliente.jpg");
  mkdirSync(dirname(assinaturaPath), { recursive: true });
  writeFileSync(assinaturaPath, Buffer.from([0xff, 0xd8, 0xff, 0xd9, 0x10]));
  equipamento.ordensServico[0].evidencias = [];
  equipamento.ordensServico[0].checklistRespostas = [];
  equipamento.ordensServico[0].assinatura.storageUrl = "/storage/os/os-assinatura-cliente/assinatura-cliente.jpg";
  const prisma = {
    cliente: {
      findFirst: async () => ({
        id: "cliente-1",
