...
test("gerarPdfPmocCliente retorna PDF com nome de arquivo e conteudo oficial", async () => {
  ...
  assert.match(pdf, /PLANO DE MANUTENCAO, OPERACAO E CONTROLE - PMOC/);
  assert.match(pdf, /Cliente\s+Maria Souza/);
  assert.match(pdf, /EMPRESA CONTRATADA/);
  assert.match(pdf, /M\. Lima Manutencoes Prediais e Industriais LTDA/);
  ...
  assert.match(pdf, /Ocupantes fixos\s+Nao informado/);
  assert.match(pdf, /Ocupantes variaveis\s+Nao informado/);
  assert.match(pdf, /DECLARACAO TECNICA E ASSINATURAS/);
  ...
  assert.ok(pdf.indexOf("(AIRMOVEBR") < pdf.indexOf("(IDENTIFICACAO DO CLIENTE"));
  assert.ok(pdf.indexOf("(IDENTIFICACAO DO CLIENTE") < pdf.indexOf("(OBJETIVO, RESPONSABILIDADES"));
  assert.ok(pdf.indexOf("(RESUMO DAS MAQUINAS") < pdf.indexOf("(MAQUINA N:001"));
  assert.ok(pdf.indexOf("(MAQUINA N:001") < pdf.indexOf("(MAQUINA N:002"));
  assert.ok(pdf.indexOf("(MAQUINA N:002") < pdf.indexOf("(DECLARACAO TECNICA E ASSINATURAS"));
  const declaracao = pdf.slice(pdf.indexOf("(DECLARACAO TECNICA E ASSINATURAS"));
