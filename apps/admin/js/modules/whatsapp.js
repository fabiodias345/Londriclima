...
whatsappView.innerHTML = \`
  ...
  <div class="whatsapp-inbox" role="region" aria-label="Central de conversas WhatsApp">
    <aside class="whatsapp-inbox-list">
      ...
      <div class="whatsapp-filters" role="tablist" aria-label="Filtro de conversas"><button class="is-active" type="button" data-whatsapp-filter="atendimento">Em Atendimento</button><button type="button" data-whatsapp-filter="aguardando">Aguardando</button><button type="button" data-whatsapp-filter="encerradas">Encerradas</button><button type="button" data-whatsapp-filter="todas">Todas</button></div>
...
function getWhatsappState(item) {
  if (item.status === "aguardando_equipe") return { key: "aguardando", className: "is-waiting-team", label: "Aguardando", ageMinutes: 0 };
...
function renderWhatsappConversations() {
  ...
  const prioridade = { atendimento: 0, aguardando: 1, encerradas: 2 }; const items = whatsappConversations.filter((item) => { const state = getWhatsappState(item); const text = \`\${item.nomeContato || ""} \${item.telefone || ""}\`.toLowerCase(); return (!query || text.includes(query)) && (filter === "todas" || filter === state.key); }).sort((a, b) => prioridade[getWhatsappState(a).key] - prioridade[getWhatsappState(b).key] || getWhatsappState(b).ageMinutes - getWhatsappState(a).ageMinutes);
...
function scheduleProposalText(conversa, date, time, equipeId, tecnicoId) { const when = new Date(date + "T12:00").toLocaleDateString("pt-BR", { weekday: "long", day: "2-digit", month: "2-digit" }); return "Olá, " + (conversa.nomeContato || "tudo bem") + ". Tenho disponibilidade " + when + " às " + time + " com " + schedulePersonLabel(equipeId, tecnicoId) + ". Pode ser? É necessário que um adulto responsável esteja no imóvel para receber a equipe."; }
...
async function loadWhatsappConversation(id) {
  ...
  const action = conversa.status === "encerrada" ? '<button class="secondary-button" data-whatsapp-action="reabrir">Reabrir conversa</button>' : conversa.status === "aguardando_equipe" ? '<button class="danger-button" data-whatsapp-action="encerrar">Encerrar</button>' : conversa.atribuidoUsuario ? '<button class="secondary-button" data-whatsapp-action="liberar">Liberar para fila</button><button class="danger-button" data-whatsapp-action="encerrar">Encerrar</button>' : '<button class="danger-button" data-whatsapp-action="encerrar">Encerrar</button>';
...
function levantamentoCustomerMessage(conversa, date, time, equipeId, tecnicoId) { const when = new Date(date + "T12:00").toLocaleDateString("pt-BR", { weekday: "long", day: "2-digit", month: "2-digit" }); return "Olá, " + (conversa.nomeContato || "tudo bem") + ". Confirmamos o levantamento técnico para " + when + " às " + time + " com " + schedulePersonLabel(equipeId, tecnicoId) + ". Nossa equipe avaliará o equipamento no local. É necessário que um adulto responsável esteja no imóvel para receber a equipe. Se precisar reagendar, avise por aqui."; }
...
document.querySelector("#whatsappConversationDetail").addEventListener("submit", async (event) => {
  ...
  const data = new FormData(form);
  const body = { tipo: "pf", nome: data.get("nome"), documento: data.get("documento"), telefone: data.get("telefone"), email: data.get("email"), cep: data.get("cep"), logradouro: data.get("logradouro"), numero: data.get("numero"), bairro: data.get("bairro"), cidade: data.get("cidade"), uf: String(data.get("uf") || "").toUpperCase() };
