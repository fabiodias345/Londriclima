...
async function loadWhatsappConversation(id) {
  if (id !== selectedWhatsappId) { whatsappSelectedScheduleDate = ""; whatsappScheduleDraft = { equipeId: "", tecnicoId: "", data: "", horario: "", proposalText: "", proposalSent: false }; whatsappScheduleOpen = false; whatsappScheduleVisibleMonth = localDate().slice(0, 7); whatsappScheduleMode = "calendar"; }
...
document.querySelector("#whatsappConversationDetail").addEventListener("click", async (event) => {
  ...
  if (action === "abrir-agenda") { whatsappScheduleOpen = true; whatsappScheduleMode = "calendar"; whatsappScheduleOptions.agenda = []; await loadWhatsappScheduleOptions(); await loadWhatsappConversation(selectedWhatsappId); return; }
