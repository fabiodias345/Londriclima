...
let whatsappScheduleDraft = { equipeId: "", tecnicoId: "", data: "", horario: "", proposalText: "", proposalSent: false };
...
function quoteForm(conversa) { const quote = conversa.orcamentos?.[0]; if (quote) { const items = quote.itens.map((item) => \`<li>\${esc(item.descricao)} <strong>\${money(item.valorTotal)}</strong></li>\`).join(""); return \`<div class="whatsapp-quote-summary"><div><span>Orçamento \${esc(quote.status)}</span><strong>\${esc(quote.titulo)}</strong><ul>\${items}</ul></div><div><b>\${money(quote.total)}</b>\${quote.status === "rascunho" ? \`<button type="button" data-whatsapp-action="enviar-orcamento" data-orcamento-id="\${esc(quote.id)}">Enviar pelo WhatsApp</button>\` : quote.status === "enviado" ? \`<span>Aguardando autorização do cliente</span>\` : quote.status === "aprovado" ? \`<span>Orçamento aprovado</span>\` : ""}</div></div>\`; } if (!whatsappCatalogItems.length) return \`<div class="whatsapp-quote-summary"><div><span>Orçamento</span><strong>Cadastre itens no catálogo antes de montar a proposta.</strong></div></div>\`; const dados = conversa.atendimento?.dados || {}; return \`<form data-whatsapp-form="orcamento" class="whatsapp-quote-form"><div class="whatsapp-workbench-title"><span>Orçamento</span><strong>Monte a proposta; a O.S. será aberta somente após o aceite.</strong></div><label class="whatsapp-quote-title">Título<input name="titulo" value="Orçamento - \${esc(dados.servico || "atendimento")}" required></label><label>Validade<input name="valido_ate" type="date"></label><div class="whatsapp-quote-lines" data-quote-lines>\${quoteItemRow()}</div><button type="button" class="secondary-button" data-quote-action="adicionar">+ Adicionar item</button><button type="submit">Salvar orçamento</button></form>\`; }
function operationalForm(conversa) { if (!conversa.cliente) return whatsappClientForm(conversa); const quote = conversa.orcamentos?.[0]; return \`\${quoteForm(conversa)}\${!quote || quote.status === "aprovado" ? scheduleForm(conversa) : ""}\`; }
...
function schedulePanel(conversa) { const os = conversa.ordemServico; const date = scheduleDateValue(os); const time = scheduleTimeValue(os); const equipeId = whatsappScheduleDraft.equipeId || os?.equipeId || ""; const tecnicoId = whatsappScheduleDraft.tecnicoId || os?.tecnicoId || ""; const ready = Boolean(equipeId || tecnicoId); const label = new Date(date + "T12:00").toLocaleDateString("pt-BR", { month: "long", year: "numeric" }); const header = '<aside class="whatsapp-schedule-panel" aria-label="Disponibilidade da agenda"><header><div><span>Disponibilidade</span><strong>Escolha uma data e horário</strong></div><button type="button" class="secondary-button" data-whatsapp-action="fechar-agenda">×</button></header>'; if (!ready) return header + '<p class="whatsapp-schedule-warning">Selecione uma equipe ou técnico antes de consultar a disponibilidade.</p></aside>'; const message = whatsappScheduleDraft.proposalSent ? "Proposta enviada. Confirme somente após o aceite do cliente." : "Escolha o horário e prepare a mensagem para o cliente."; return header + '<div class="whatsapp-schedule-month"><button type="button" class="secondary-button" data-whatsapp-action="mes-anterior">‹</button><strong>' + label.charAt(0).toUpperCase() + label.slice(1) + '</strong><button type="button" class="secondary-button" data-whatsapp-action="mes-proximo">›</button></div><div class="whatsapp-schedule-weekdays"><span>Dom</span><span>Seg</span><span>Ter</span><span>Qua</span><span>Qui</span><span>Sex</span><span>Sáb</span></div><div class="whatsapp-schedule-calendar">' + scheduleCalendar(date) + '</div><div class="whatsapp-schedule-slots">' + scheduleSlots(date, equipeId, tecnicoId, os?.id, time) + '</div><div class="whatsapp-schedule-actions"><p>' + message + '</p><button type="button" class="secondary-button" data-whatsapp-action="propor-agendamento">Preparar mensagem</button><button type="button" data-whatsapp-action="confirmar-agendamento" ' + (whatsappScheduleDraft.proposalSent ? "" : "disabled") + '>Confirmar agendamento</button></div></aside>'; }
...
async function loadWhatsappConversation(id) {
  if (id !== selectedWhatsappId) { whatsappSelectedScheduleDate = ""; whatsappScheduleDraft = { equipeId: "", tecnicoId: "", data: "", horario: "", proposalText: "", proposalSent: false }; whatsappScheduleOpen = false; whatsappScheduleVisibleMonth = localDate().slice(0, 7); }
...
document.querySelector("#whatsappConversationDetail").addEventListener("click", async (event) => {
  ...
  if (action === "abrir-agenda") { whatsappScheduleOpen = true; whatsappScheduleOptions.agenda = []; await loadWhatsappScheduleOptions(); await loadWhatsappConversation(selectedWhatsappId); return; }
  if (action === "fechar-agenda") { whatsappScheduleOpen = false; await loadWhatsappConversation(selectedWhatsappId); return; }
  ...
  if (target.dataset.whatsappScheduleDate) { whatsappScheduleDraft.data = target.dataset.whatsappScheduleDate; whatsappScheduleVisibleMonth = whatsappScheduleDraft.data.slice(0, 7); whatsappScheduleDraft.proposalSent = false; await loadWhatsappConversation(selectedWhatsappId); return; }
  ...
  if (action === "confirmar-agendamento") { await confirmWhatsappSchedule(); }
});
