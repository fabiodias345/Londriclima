...
function operationalForm(conversa) { const ai = aiWorkbench(); if (!conversa.cliente) return ai + whatsappClientResolutionForm(conversa); if (whatsappServiceChoiceRequired) return ai + serviceChoiceForm(); if (conversa.levantamento) return ai + levantamentoForm(conversa); const quote = conversa.orcamentos?.[0]; if (whatsappServiceMode === "levantamento") return ai + levantamentoForm(conversa); if (whatsappServiceMode === "orcamento" || quote) return ai + \`\${quoteForm(conversa)}\${!quote || quote.status === "aprovado" ? scheduleForm(conversa) : ""}\`; return ai + serviceChoiceForm(); }
