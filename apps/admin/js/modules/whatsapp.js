...
function serviceChoiceForm() { return '<section class="whatsapp-service-choice"><div class="whatsapp-workbench-title"><span>Próximo passo</span><strong>Escolha como este atendimento deve continuar.</strong></div><div class="whatsapp-service-choice-actions"><button type="button" data-whatsapp-action="escolher-levantamento"><strong>Levantamento técnico</strong><small>Agendar visita para diagnóstico, sem preço e sem O.S.</small></button><button type="button" data-whatsapp-action="escolher-orcamento"><strong>Orçamento direto</strong><small>Montar proposta com itens, valores e validade.</small></button></div></section>'; }
function operationalForm(conversa) { if (!conversa.cliente) return whatsappClientForm(conversa); if (conversa.levantamento) return levantamentoForm(conversa); const quote = conversa.orcamentos?.[0]; if (whatsappServiceMode === "levantamento") return levantamentoForm(conversa); if (whatsappServiceMode === "orcamento" || quote) return \`\${quoteForm(conversa)}\${!quote || quote.status === "aprovado" ? scheduleForm(conversa) : ""}\`; return serviceChoiceForm(); }
...
async function loadWhatsappConversation(id) {
  if (id !== selectedWhatsappId) { whatsappSelectedScheduleDate = ""; whatsappScheduleDraft = { equipeId: "", tecnicoId: "", data: "", horario: "", proposalText: "", proposalSent: false, context: "os" }; whatsappServiceMode = ""; whatsappScheduleOpen = false; whatsappScheduleVisibleMonth = localDate().slice(0, 7); whatsappScheduleMode = "calendar"; }
...
document.querySelector("#whatsappConversationDetail").addEventListener("click", async (event) => {
  ...
  if (action === "escolher-levantamento" || action === "escolher-orcamento") { whatsappServiceMode = action === "escolher-levantamento" ? "levantamento" : "orcamento"; await loadWhatsappConversation(selectedWhatsappId); return; }
