...
function whatsappClientForm(conversa) { const dados = conversa.atendimento?.dados || {}; return \`<form data-whatsapp-form="cliente" class="whatsapp-workbench-form"><div class="whatsapp-workbench-title"><span>Cadastro e O.S.</span><strong>Complete uma vez; a O.S. será criada automaticamente.</strong></div><label>Nome<input name="nome" value="\${esc(dados.nome || conversa.nomeContato || "")}" required></label><label>Telefone<input name="telefone" value="\${esc(conversa.telefone || "")}"></label><label>Cidade<input name="cidade" value="\${esc(dados.cidade_bairro || "")}" required></label><label>UF<input name="uf" value="PR" maxlength="2" required></label><label class="whatsapp-field-wide">Endereço<input name="logradouro" placeholder="Rua, avenida ou rodovia"></label><label>Número<input name="numero" placeholder="Nº"></label><label>Bairro<input name="bairro" placeholder="Bairro"></label><label>CEP<input name="cep" placeholder="00000-000"></label><button type="submit">Criar cliente e O.S.</button></form>\`; }
...
async function loadWhatsappConversation(id) {
  ...
  const conversa = await response.json(); await loadWhatsappScheduleOptions(); selectedWhatsappId = id; void fetch(\`\${apiBaseUrl}/admin/whatsapp/conversas/\${id}/ler\`, { method: "PATCH", headers: authHeaders() });
  ...
  const form = conversa.cliente ? scheduleForm(conversa) : whatsappClientForm(conversa);
