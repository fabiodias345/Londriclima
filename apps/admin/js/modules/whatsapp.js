...
function renderWhatsappConversations() {
  const list = document.querySelector("#whatsappConversationList"); const query = String(document.querySelector("#whatsappSearchInput")?.value || "").toLowerCase().trim(); const filter = document.querySelector("[data-whatsapp-filter].is-active")?.dataset.whatsappFilter || "todas";
...
function whatsappClientForm(conversa) { const dados = conversa.atendimento?.dados || {}; return \`<form data-whatsapp-form="cliente" class="whatsapp-workbench-form"><div class="whatsapp-workbench-title"><span>Pré-cadastro</span><strong>Crie o cliente para montar o orçamento. A O.S. só será aberta após a aprovação.</strong></div><label>Nome<input name="nome" value="\${esc(dados.nome || conversa.nomeContato || "")}" required></label><label>Telefone<input name="telefone" value="\${esc(conversa.telefone || "")}"></label><label>Cidade<input name="cidade" value="\${esc(dados.cidade_bairro || "")}" required></label><label>UF<input name="uf" value="PR" maxlength="2" required></label><label class="whatsapp-field-wide">Endereço<input name="logradouro" placeholder="Rua, avenida ou rodovia"></label><label>Número<input name="numero" placeholder="Nº"></label><label>Bairro<input name="bairro" placeholder="Bairro"></label><label>CPF ou RG<input name="documento" placeholder="CPF ou RG" required></label><label>CEP<input name="cep" placeholder="00000-000"></label><button type="submit">Criar cliente</button></form>\`; }
...
function quoteForm(conversa) { const quote = conversa.orcamentos?.[0]; if (quote) { const items = quote.itens.map((item) => \`<li>\${esc(item.descricao)} <strong>\${money(item.valorTotal)}</strong></li>\`).join(""); return \`<div class="whatsapp-quote-summary"><div><span>Orçamento \${esc(quote.status)}</span><strong>\${esc(quote.titulo)}</strong><ul>\${items}</ul></div><div><b>\${money(quote.total)}</b>\${quote.status === "rascunho" ? \`<button type="button" data-whatsapp-action="enviar-orcamento" data-orcamento-id="\${esc(quote.id)}">Enviar pelo WhatsApp</button>\` : ""}</div></div>\`; } if (!whatsappCatalogItems.length) return \`<div class="whatsapp-quote-summary"><div><span>Orçamento</span><strong>Cadastre itens no catálogo antes de montar a proposta.</strong></div></div>\`; const dados = conversa.atendimento?.dados || {}; return \`<form data-whatsapp-form="orcamento" class="whatsapp-quote-form"><div class="whatsapp-workbench-title"><span>Orçamento</span><strong>Monte a proposta; a O.S. será aberta somente após o aceite.</strong></div><label class="whatsapp-quote-title">Título<input name="titulo" value="Orçamento - \${esc(dados.servico || "atendimento")}" required></label><label>Validade<input name="valido_ate" type="date"></label><div class="whatsapp-quote-lines" data-quote-lines>\${quoteItemRow()}</div><button type="button" class="secondary-button" data-quote-action="adicionar">+ Adicionar item</button><button type="submit">Salvar orçamento</button></form>\`; }
function operationalForm(conversa) { return conversa.cliente ? quoteForm(conversa) : whatsappClientForm(conversa); }
function scheduleForm(conversa) { const os = conversa.ordemServico; const dateValue = whatsappSelectedScheduleDate || (os?.agendadaPara ? localDate(os.agendadaPara) : localDate()); const equipeId = whatsappScheduleDraft.equipeId || os?.equipeId; const tecnicoId = whatsappScheduleDraft.tecnicoId || os?.tecnicoId; const timeValue = os?.agendadaPara ? new Date(os.agendadaPara).toLocaleTimeString("pt-BR", { hour: "2-digit", minute: "2-digit", hour12: false }) : "07:00"; const previa = conversa.atendimento?.previaOs || {}; return \`<form data-whatsapp-form="os" class="whatsapp-workbench-form whatsapp-schedule-form"><div class="whatsapp-workbench-title"><span>O.S. \${os ? "criada" : "pendente"}</span><strong>\${esc(os?.titulo || previa.titulo || "Atendimento WhatsApp")}</strong></div><label class="whatsapp-field-wide">Título<input name="titulo" value="\${esc(os?.titulo || previa.titulo || "")}" required></label><label class="whatsapp-field-wide">Detalhes<textarea name="detalhes" rows="1">\${esc(previa.detalhes || "")}</textarea></label><label>Data<input name="data" type="date" value="\${dateValue}" required></label><label>Horário<select name="horario" required>\${scheduleHours(dateValue, equipeId, tecnicoId, os?.id, timeValue)}</select></label><label>Equipe<select name="equipe_id">\${scheduleOptions("equipe", whatsappScheduleOptions.equipes, equipeId)}</select></label><label>Técnico<select name="tecnico_id">\${scheduleOptions("tecnico", whatsappScheduleOptions.tecnicos.filter((item) => item.role !== "admin"), tecnicoId)}</select></label><div class="whatsapp-agenda-day">\${dayAgenda(dateValue)}</div><button type="submit">Salvar agendamento</button></form>\`; }

async function loadWhatsappConversation(id) {
  if (id !== selectedWhatsappId) { whatsappSelectedScheduleDate = ""; whatsappScheduleDraft = { equipeId: "", tecnicoId: "" }; }
...
document.querySelector("#whatsappConversationDetail").addEventListener("click", async (event) => { const action = event.target.dataset.whatsappAction; if (action === "enviar-orcamento") { const response = await fetch(\`\${apiBaseUrl}/admin/comercial/orcamentos/\${event.target.dataset.orcamentoId}/enviar\`, { method: "POST", headers: authHeaders() }); if (!response.ok) { window.alert("Não foi possível enviar o orçamento."); return; } await loadWhatsappConversation(selectedWhatsappId); return; } if (action) await whatsappAction(action); });
...
document.querySelector("#whatsappConversationDetail").addEventListener("change", (event) => { if (["data", "equipe_id", "tecnico_id"].includes(event.target.name)) { if (event.target.name === "data") whatsappSelectedScheduleDate = event.target.value; if (event.target.name === "equipe_id") whatsappScheduleDraft.equipeId = event.target.value; if (event.target.name === "tecnico_id") whatsappScheduleDraft.tecnicoId = event.target.value; void loadWhatsappConversation(selectedWhatsappId); } });
