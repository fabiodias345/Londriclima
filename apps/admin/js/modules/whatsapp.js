...
function operationalForm(conversa) { if (!conversa.cliente) return whatsappClientForm(conversa); if (whatsappServiceChoiceRequired) return serviceChoiceForm(); if (conversa.levantamento) return levantamentoForm(conversa); const quote = conversa.orcamentos?.[0]; if (whatsappServiceMode === "levantamento") return levantamentoForm(conversa); if (whatsappServiceMode === "orcamento" || quote) return \`\${quoteForm(conversa)}\${!quote || quote.status === "aprovado" ? scheduleForm(conversa) : ""}\`; return serviceChoiceForm(); }
