...
whatsappSummary.innerHTML = '<article class="os-summary-card os-summary-green"><span>Aguardando atendente</span><strong id="whatsappOpenCount">0</strong><small>Transferências do Bolt</small></article>';
...
async function loadWhatsappConversations() {
  ...
  if (!response.ok) { status.textContent = "Não foi possível carregar as conversas."; return; }
  ...
  status.textContent = whatsappConversations.length ? `${whatsappConversations.length} conversa(s)` : "Nenhuma conversa recebida."; renderWhatsappConversations();
}

function renderWhatsappConversations() {
  const list = document.querySelector("#whatsappConversationList");
  list.innerHTML = whatsappConversations.map((item) => { const last = item.mensagens?.[0]?.texto || "Sem mensagens"; const pending = item.status === "humano" && !item.atribuidoUsuarioId; return `<button class="os-compact-card ${pending ? "os-card-attention" : ""}" type="button" data-whatsapp-id="${esc(item.id)}"><strong>${esc(item.nomeContato || item.telefone)}</strong><span>${pending ? "Aguardando atendente" : item.status === "humano" ? `Atendimento: ${esc(item.atribuidoUsuario?.nome || "humano")}` : item.status === "encerrada" ? "Encerrada" : "Bolt"}</span><small>${esc(last.slice(0, 90))}</small></button>`; }).join("");
}
