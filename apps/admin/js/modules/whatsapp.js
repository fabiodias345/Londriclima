...
async function loadWhatsappConversation(id) {
  ...
  const action = (conversa.status === "encerrada" ? \`<button class="secondary-button" data-whatsapp-action="reabrir">Reabrir conversa</button>\` : conversa.atribuidoUsuario ? \`<button class="secondary-button" data-whatsapp-action="liberar">Liberar para fila</button><button class="danger-button" data-whatsapp-action="encerrar">Encerrar</button>\` : \`<button class="danger-button" data-whatsapp-action="encerrar">Encerrar</button>\`) + \`<button class="danger-button" data-whatsapp-action="apagar">Apagar conversa</button>\`;
...
async function whatsappAction(action) {
  if (action === "apagar" && !window.confirm("Apagar esta conversa e todo o historico? A proxima mensagem deste numero iniciara um novo atendimento.")) return;
  const path = action === "liberar" ? "liberar" : action === "reabrir" ? "reabrir" : action === "encerrar" ? "encerrar" : "";
  const method = action === "apagar" ? "DELETE" : action === "encerrar" || action === "reabrir" ? "POST" : "PATCH";
  const url = apiBaseUrl + "/admin/whatsapp/conversas/" + selectedWhatsappId + (path ? "/" + path : "");
  const response = await fetch(url, { method, headers: { ...authHeaders(), "Content-Type": "application/json" }, body: action === "encerrar" ? JSON.stringify({ motivo: "concluido" }) : undefined });
  if (!response.ok) { window.alert("Nao foi possivel concluir esta acao."); return; }
  if (action === "apagar") {
    selectedWhatsappId = "";
    document.querySelector("#whatsappConversationDetail").innerHTML = '<div class="whatsapp-empty-state"><span class="whatsapp-empty-icon">&#9673;</span><strong>Conversa apagada</strong><p>Envie uma nova mensagem pelo WhatsApp para iniciar o atendimento novamente.</p></div>';
