...
└── schema.sql                   ← Pendente: schema do banco PostgreSQL
...
| `schema.sql` | Pendente: schema PostgreSQL com ENUMs, relacionamentos e coluna `empresa_id`. |
...
# 2. Crie e configure o banco de dados quando schema.sql estiver disponível
psql -U postgres -c "CREATE DATABASE londriclima_dev;"
psql -U postgres -d londriclima_dev -f schema.sql

# 3. Configure as variáveis de ambiente do backend
cd londriclima-backend
cp .env.example .env
...
```text
1. londriclima-backend   → Schema, autenticação, endpoints base e empresa_id global.
2. londriclima-mobile    → Estrutura Flutter + Drift + fluxo offline da OS.
3. londriclima-admin     → Painel React consumindo a API do backend.
